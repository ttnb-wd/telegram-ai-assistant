...
telegram = TelegramClient(StringSession(SESSION), API_ID, API_HASH)
...
async def owner_message(event):
    ...
    print("OWNER ACTIVE:", event.chat_id, flush=True)
...
async def handler(event):
    ...
    save_message(chat_id, "user", message, username=str(event.sender.username))

    if chat_id in owner_activity:
        if time.time() - owner_activity[chat_id] < WAIT_TIME:
            print("Owner is chatting. AI sleeping...", flush=True)
            return
...
"""
    ...
    try:
        ...
        reply = response.text or "ခဏနော် 😅"

        await event.reply(reply)
        save_message(chat_id, "assistant", reply, userName="Metro AI")
        print("\nAI:", reply, flush=True)
...
# Start Bot
# ==========================
print("🤖 Metro AI Starting...", flush=True)
telegram.start()
print("✅ Telegram Connected\n✅ Memory System Active\n🚀 AI Assistant Running", flush=True)
